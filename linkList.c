...
struct Student* create(){
	...
	printf("please first input name ,then seno(The program will be end,when seno is zero. )\n");
	scanf("%s",&pNew->name);
	scanf("%d",&pNew->seno);
...
// 在链表中删除结点
// index : seno
void delete(int index,struct Student* head){
	int i;
	struct Student* sTmp;
	struct Student* sPre;		// 删除结点的前一个结点
	sTmp = head;
	sPre = sTmp;

	for(i=0;i<link_length;i++){	// 循环使得sTmp指向index结点
		sPre = sTmp;
		sTmp = sTmp->pNext;
		if(sTmp->seno == index){		// 
			break;
